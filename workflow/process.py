...
def parse_query_value(query_str):
    ...
    try:
        ...
        if (query_str == 'now' or query_str == '' ):
            d = now()

        else:
            # Parse datetime string or timestamp
            try:
                d = epoch(float(query_str)/1000+_offset)
            except ValueError:
...
def alfred_items_for_value(value):
    ...
    timestamp = calendar.timegm(value.datetime.utctimetuple())
    item_value = timestamp * 1000
    ...
    for format, description in formats:
        ...
        # use local timezone
        item_value = time.strftime(format, time.localtime(timestamp))
        results.append(alfred.Item(
            title=str(item_value),
